...
# Perform Gene Ontology enrichment analysis using the enrichGO function from 
# the "clusterProfiler" package
clusterProfiler_IFNg_vs_naive <- enrichGO(
...
Additionally, as full dataset contains roughly 4600 gene sets, which is appropriate for real analyses but unnecessarily large for demonstration purposes, we truncated the data to the first 500 gene sets.


```{r renamecolumns, eval=F}
# We're preparing the data for GeDi using the 'path_to_GeDi' function from the package.
clusterProfiler_IFNg_vs_naive <- clusterProfiler_IFNg_vs_naive@result
macrophage_example <- path_to_GeDi(clusterProfiler_IFNg_vs_naive,
                                   enrichment_package = "clusterProfiler")

macrophage_example <- macrophage_example[1:500, ]
```

After we have preprocessed the data, the data is now ready to be used in `GeDi`.


# Running `GeDi` on the dataset

Now we can start to explore our data using `GeDi`. For this you can either follow the chunks in this document to prepare the data or we can load the prepared object provided with the repository of this document [Github](https://github.com/AnnekathrinSilvia/GeDi_supplement) .
